...
def _get_secret_key():
    ...
    if env_key:
        return env_key
    # Use the same DATA_DIR as the database so the key persists on Railway volumes
    ...
    try:
        ...
        # Fallback: stable key derived from service ID so sessions survive restarts
        import hashlib as _hl
        _svc = os.environ.get('RAILWAY_SERVICE_ID', 'floodclaim-pro-default')
        return _hl.sha256(f'floodclaim-secret-{_svc}'.encode()).hexdigest()
...
app.config['SESSION_COOKIE_HTTPONLY']    = True
app.config['SESSION_COOKIE_SAMESITE']   = 'Lax'
# Secure=False set above — Railway edge terminates TLS, plain HTTP to container
